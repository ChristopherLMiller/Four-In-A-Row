...
{
	m_pIntroTexture = NULL;
	mSprite = NULL;
}

void IntroState::destroy(Connect4 *pGame)
{
	ReleaseCOM(m_pIntroTexture);
	ReleaseCOM(mSprite);
}

BOOL IntroState::create(Connect4 *pGame)
{
	D3DXCreateTextureFromFile(gd3dDevice, "Intro.jpg", &m_pIntroTexture);
...
void IntroState::checkInput(Connect4 *pGame)
{
	// poll the device
	gDInput->poll();
...
void IntroState::update(Connect4 *pGame)
{
	// increment delay once every loop
	delay += 0.1f;
...
void IntroState::display(Connect4 *pGame)
{
	...
	HR(mSprite->Draw(m_pIntroTexture, &mBkgrdRect, &mBkgrdCntr, 0, D3DCOLOR_XRGB(255,255,255)));
	HR(mSprite->Flush());
...
void IntroState::onResetDevice(Connect4 *pGame, HWND mhMainWnd)
{
...
void IntroState::onLostDevice(Connect4 *pGame)
{
